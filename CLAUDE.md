...
- **GitHub Flavored Markdown (GFM)** の仕様に厳密に準拠する
- 見出し、段落、リスト、テーブル、コードブロックなどの**すべてのブロック要素の前後に必ず1行の空行**を挿入する
- リストやチェックボックスは標準的な構文（`- [ ]` など）を使用し、独自記法は行わない
- レンダリング時の正確な表示を最優先し、**行間を詰めるなどの独自の最適化や改行省略は一切行わない**
